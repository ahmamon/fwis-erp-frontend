...
function ReportView({ report, onClose }) {
  ...
  const printReport = () => {
    ...
    const brandBase = `${window.location.origin}/export-branding`;
    popup.document.write(`<!doctype html><html><head><title>${escapeHtml(report.student.name)} performance report</title><style>body{font-family:Arial,sans-serif;color:#1c2733;margin:40px;line-height:1.5}.brand{display:grid;grid-template-columns:120px 1fr 88px;gap:20px;align-items:center}.brand img{display:block;max-width:100%;max-height:78px;margin:auto}.brand-copy{text-align:center}header{border-bottom:3px solid #c6a15b;padding-bottom:16px}h1{color:#0b1f3a;margin:6px 0}h2{color:#1b3a63;margin-top:26px}small{color:#5b6472}li{margin:5px 0}@media print{button{display:none}.brand{break-inside:avoid}}</style></head><body><header><div class="brand"><img src="${brandBase}/fwis-logo.png" alt="FWIS school logo"><div class="brand-copy"><small>FUTURE WINDOW INTERNATIONAL SCHOOL</small><h1>Student Performance Report</h1></div><img src="${brandBase}/cognia-accredited.png" alt="Cognia Accredited"></div><p><b>${escapeHtml(report.student.name)}</b> · ${escapeHtml(report.student.grade?.label)} ${escapeHtml(report.student.classSection)}</p></header>${domain("Academic", report.academic)}${domain("Behavior", report.behavior)}${eleot}<p><small>Generated ${new Date().toLocaleString()}</small></p><button onclick="window.print()">Print / Save as PDF</button></body></html>`);
