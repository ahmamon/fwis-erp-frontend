...
export default function AdminPanel({ currentUser, onNavigate }) {
  ...
  return (
    <div style={{ padding: "20px 28px 60px", maxWidth: 1100, margin: "0 auto" }}>
      <h1 style={{ fontFamily: "Georgia, serif", fontSize: 20, color: T.navy900, margin: "0 0 6px" }}>Admin Control Panel</h1>
      <div style={{ fontSize: 12.5, color: T.ink600, marginBottom: 16 }}>
        Staff, school data, audit trail and reports — admin access only.
      </div>
      ...
      {tab === "branches" && <SimplePanel title="Branches" endpoint="branches" fields={[["name", "Name"], ["code", "Code"], ["location", "Location"]]} labelOf={(b) => b.name || b.code} />}
      {tab === "subjects" && <SimplePanel title="Subjects" endpoint="subjects" fields={[["name", "Name"]]} labelOf={(s) => s.name} />}
      {tab === "grades" && <SimplePanel title="Grade Bands" endpoint="grade-bands" fields={[["label", "Label"]]} labelOf={(g) => g.label} />}
  ...
  return (
    <SectionCard title="Audit Log">
      <div style={{ fontSize: 12.5, color: T.ink600, marginBottom: 12 }}>
        The 100 most recent actions across the system — plan lifecycles and staff management.
      </div>
      {rows.length === 0 ? (
        <div style={{ color: T.ink600, fontSize: 13.5, padding: 8 }}>No activity recorded yet.</div>
      ) : (
...
function ReportsPanel({ onNavigate }) {
  return (
    <SectionCard title="Report Center">
      <div style={{ fontSize: 12.5, color: T.ink600, marginBottom: 12 }}>
        Jump to the full Report Center for these views.
      </div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
        {REPORT_TILES.map((t) => (
          <Button key={t.key} variant="outline" onClick={() => onNavigate("reports")}>{t.label}</Button>
        ))}
      </div>
    </SectionCard>
  );
...
function NotesPanel() {
  ...
  const audience = (n) => [
    n.targetRole ? ROLE_LABELS[n.targetRole] : "Everyone",
    n.department && `Department: ${n.department}`,
  ].filter(Boolean).join(" · ");

  return (
    <SectionCard title="Notes & Announcements">
      <div style={{ fontSize: 12.5, color: T.ink600, marginBottom: 16 }}>
        Post short announcements that surface on staff dashboards. People see them until they expire or are paused.
      </div>
      <ErrorBanner message={error} />

      {/* Create */}
      <div style={{ border: `1px solid ${T.line}`, borderRadius: 12, padding: 16, background: T.cream100, marginBottom: 18 }}>
        <FieldLabel required>Announcement text</FieldLabel>
        <Input value={text} onChange={setText} placeholder="e.g. Staff meeting Tuesday 3:30pm in the library" />
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 12, alignItems: "flex-end" }}>
          <div style={{ flex: "1 1 150px" }}>
            <FieldLabel>Audience</FieldLabel>
            <Select value={targetRole} onChange={setTargetRole} placeholder="Everyone" options={ROLE_OPTIONS} />
          </div>
          <div style={{ flex: "1 1 150px" }}>
            <FieldLabel>Department (optional)</FieldLabel>
            <Input value={department} onChange={setDepartment} placeholder="e.g. English" />
          </div>
          <div style={{ flex: "1 1 150px" }}>
            <FieldLabel>Expires (optional)</FieldLabel>
            <Input type="date" value={expiresAt} onChange={setExpiresAt} />
          </div>
          <Button onClick={createNote} disabled={!text.trim()}>Post announcement</Button>
        </div>
      </div>

      {/* List — admins see every note, paused or expired, so they can re-enable. */}
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {notes.length === 0 && <div style={{ color: T.ink600, fontSize: 13.5, padding: 8 }}>Nothing posted yet.</div>}
        {notes.map((n) =>
          editingId === n.id ? (
            <div key={n.id} style={{ border: `1px solid ${T.line}`, borderRadius: 10, padding: 12, background: "#fff" }}>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 10, alignItems: "flex-end" }}>
                <div style={{ flex: "1 1 140px" }}>
                  <FieldLabel>Audience</FieldLabel>
                  <Select value={draft.targetRole || ""} onChange={(v) => setDraft({ ...draft, targetRole: v })} placeholder="Everyone" options={ROLE_OPTIONS} />
                </div>
                <div style={{ flex: "1 1 140px" }}>
                  <FieldLabel>Department</FieldLabel>
                  <Input value={draft.department || ""} onChange={(v) => setDraft({ ...draft, department: v })} placeholder="Optional" />
                </div>
                <div style={{ flex: "1 1 140px" }}>
                ...
                <Button onClick={saveEdit} style={{ padding: "6px 12px" }}>Save</Button>
                <Button variant="outline" onClick={() => setEditingId(null)} style={{ padding: "6px 12px" }}>Cancel</Button>
              </div>
            </div>
          ) : (
            <div key={n.id} style={{ border: `1px solid ${T.line}`, borderRadius: 10, padding: 12, background: "#fff", display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start" }}>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 13.5, fontWeight: 600, color: T.ink900, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                  {n.text}
                  {!n.active && (
                    <span style={{ fontSize: 10.5, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5, background: "#EFEDE7", color: T.ink600, borderRadius: 999, padding: "2px 8px" }}>Paused</span>
                  )}
                </div>
                <div style={{ fontSize: 12, color: T.ink600, marginTop: 3 }}>
                  {n.authorName} · {new Date(n.createdAt).toLocaleDateString()} · {audience(n)}
                  {n.expiresAt ? ` · Expires ${new Date(n.expiresAt).toLocaleDateString()}` : " · No expiry"}
                </div>
              </div>
              <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                <Button variant="outline" onClick={() => { setDraft({ ...n, expiresAt: toDate(n.expiresAt) }); setEditingId(n.id); }} style={{ padding: "6px 12px" }}>Edit</Button>
                <Button variant="danger" onClick={() => removeNote(n)} style={{ padding: "6px 12px" }}>Delete</Button>
              </div>
            </div>
          )
        )}
